...
def get_hrd_counseling_data():
    ...
    (
        ...
        where s.training_id in
            (select training_id from training where trainee_key_allowed = 0)
    ) b
    where c.student_id = b.student_id
    order by c.counseling_date, b.training_id
    """

    counsel_df = pd.read_sql(query, engine)
...
def get_counsel_feature() :
    counsel_df = get_hrd_counseling_data()
